...
var config = {
	...
	url: {
		...
		redirection: "http://govip.parseapp.com/comprar/sucesso",
		notification: "http://govip.parseapp.com/payment/notification"
	}
}
...
function PagSeguroModule(config) {
	...
	if(!config.email) {
		...
		throw new Error("PagSeguro currency config undefined");
	}
		...
		this.xml = '<?xml version="1.0" encoding="ISO-8859-1" standalone="yes">';

		this.obj = new Object();
		this.obj['currency'] = config.currrency || "BRL";
...
PagSeguroModule.prototype.setBuyer = function(buyer) {
	...
	this.obj.sender['name'] = buyer.name;
	this.obj.sender['email'] = buyer.email;
...
PagSeguroModule.prototype.send = function(fn) {
	...
	Parse.Cloud.httpRequest({
		...
		success: function(httpResponse) {
			...
			fn(null, x2js.xml2json(httpResponse.text));
		},

		error: function(httpResponse) {
			console.log(httpResponse.text);
			fn(x2js.xml2json(httpResponse.text));
		}
	});
...
console.log("Pagseguro token: " + config.token);

var pagseguro = new PagSeguroModule(config);
var Order = Parse.Object.extend("Order");
var Attendee = Parse.Object.extend("Attendee");

Parse.Cloud.define("paymentGateway", function(request, response) {

	var user = Parse.User.current();
	pagseguro.setReference(request.params.order);
	console.log("Preparing order with reference: " + request.params.order);
	...
	query.get(request.params.order, {
		...
		success: function(order) {
			...
			console.log("querying for attendees");
			...
			query.find({
				...
				success: function(list) {
					...
					console.log("preparing pagseguro items");

					for(var i; i < list.length; i++) {

						pagseguro.addItem({
							id: list[i].id,
							description: list[i].get("description"),
							amount: list[i].get("value"),
							quantity: 1
						});
					}

					pagseguro.send(function(err, payment) {
